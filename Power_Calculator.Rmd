...
temp = paste0(getwd(),"/SimMethyl_run_",item,".zip")
...
plot_line = function(data,parameter_modified_string){ # input for parametere_modified should be a string
...
```{r setup, include=FALSE}
heat_map_t_test = function(){
  data_zip15 = merge_data(15)
  
  data_zip_selected = subset(data_zip15, select = c(test,Power,n_samples,effect_size))
  data_zip_selected$Power = as.numeric(format(data_zip_selected$Power, digits = 2))
  data_slected_t_test = data_zip_selected[data_zip_selected$test == "t_test",]
data_slected_test %>% ggplot(aes(x= n_samples, y = effect_size, fill = Power)) +
...
```{r setup, include=FALSE}
heat_map = function(n_zip,x_parameter_string,y_parameter_string,color_string,test_string){
  all_zips = merge_data(n_zip)
  columns_selected = all_zips[,names(all_zips) %in% c("test",color_string,x_parameter_string,y_parameter_string)]
  columns_selected[,1] = as.numeric(format(columns_selected[,1], digits = 2))
  columns_selected = columns_selected[columns_selected[,2] == test_string,]
fig = columns_selected %>% ggplot(aes(x = columns_selected[,3], y = columns_selected[,4], fill = columns_selected[,1])) +
 geom_tile(color = "white", lwd=1) +
  geom_text(aes(label = columns_selected[,1]), color = "white", size = 5) 
return(fig)
}
```
...
data_slected_test %>% ggplot(aes(x= data_slected_test[,3], y = data_slected_test[,4], fill = data_slected_test[,2])) +
 geom_tile(color = "white", lwd=1) +
  geom_text(aes(label = data_slected_test[,2]), color = "white", size = 5) 
