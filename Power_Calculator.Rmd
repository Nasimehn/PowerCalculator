...
```{r setup, include=FALSE}
extract_data_from_zip = function(all_runs_zip, zip_order_num, file_name){
...
extracted_data = all_runs_zip[[ zip_order_num]][[file_name_to_extract]]
...
Simulated_data <- extract_data_from_zip(run2,2,"Simulated_data")
...
```{r setup, include=FALSE}
get_healthy_proportion = function(all_zips, zip_file_order, simulated_data_frame){
...
get_healthy_proportion(all_zips,2, Simulated_data)
...
silmulated_data_frame = extract_data_from_zip(all_zips,zip_file_order,"Simulated_data")
n_samples_per_gr = get_num_samp_per_group(all_zips,zip_file_order,silmulated_data)
n_group1 = n_samples_per_gr[1]

store_all_p_val= c()
for(i in nrow(silmulated_data_frame)){
t_test_output = t.test(silmulated_data_frame[i,1:n_group1],silmulated_data_frame[i,(n_group1+1):ncol(silmulated_data_frame)])
...
Ks_test = function(all_zips,zip_file_order){
silmulated_data_frame = extract_data_from_zip(all_zips,zip_file_order,"Simulated_data")
n_samples_per_gr = get_num_samp_per_group(all_zips,zip_file_order,silmulated_data)
n_group1 = n_samples_per_gr[1]
...
store_all_p_val= c()
for(i in nrow(silmulated_data_frame)){
  gr1 = as.numeric(silmulated_data_frame[i,1:n_group1])
  gr2 = as.numeric(silmulated_data_frame[i,(n_group1+1):ncol(silmulated_data_frame)])
KS_test_output = ks.test(gr1,gr2,alternative = "two.sided")
...
limma_test = function(all_zips,zip_file_order){
