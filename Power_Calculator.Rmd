...
```{r setup, include=FALSE}
get_healthy_proportion = function(all_zips, zip_file){
user_parameters = extract_data_from_zip(all_zips, zip_file, "User_Parameters")
healthy_proportion = user_parameters$Value[user_parameters$Parameter == "Healthy proportion"]
n_Group1 = ncol(Simulated_data)*healthy_proportion
n_Group2 = ncol(Simulated_data)*1-healthy_proportion
...
get_healthy_proportion(run2,1)

```

```{r setup, include=FALSE}

local_t_test = function(Simulated_data){
  for 
} 

 t_test_output = t.test(Simulated_data[,1:30],Simulated_data[, 30:ncol(Simulated_data)])
```
...
t_test_output <- cpg.assoc(Simulated_data, indep = phenotype$Group, logit.transform = TRUE) #indep = A vector containing the variable to be tested for association #logit.transform = transforming B-values to M-values 
