...
#data = merge_data(all_zips)
data = data
as.numeric(data$n_samples)
gg_plot = 
  n_CpGs = 
  #y =  4/5*max(as.numeric(data$n_samples))
  #x = max(data$Power)
  ggplot(data, aes(x=Power , y=as.numeric(n_samples), color = test)) +
    geom_line()+
  ...
  annotate(geom= "text", x = 1, y = 120, label= "effect size ")
