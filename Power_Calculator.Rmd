...
power_calc_multiple_runs = function(all_zips){
  df_all_test = as.data.frame(matrix(data=NA, nrow=1,ncol=3))
  names(df_all_test) = c("t_test","KS_test", "limma")
  for (i in 1:all_zips){
...
#df <- data.frame(Samples, Statistical_power)

generate_visualisation = function(all_zips){
data = combined_df(all_zips)
gg_plot = ggplot(data, aes(x=limma , y=n_samples)) +
    geom_point()+
  theme_minimal()
return(gg_plot)
}
...
```{r setup, include=FALSE}
ex_data = data[1:3,]
gg_plot = ggplot(ex_data, aes(x=limma , y=n_samples)) +
    geom_point()+
  theme_minimal()
print(gg_plot)
